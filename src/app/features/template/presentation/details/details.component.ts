...
import { IQueryTemplate, ITemplate, ITemplateFilter } from '@templates';
...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.template$ = this.fStore.seeTemplate();
    this.initialProsses();
    this.get();
  }
  ...
  get(): void {
    const query: IQueryTemplate = {
      filter: this.filter,
    };

    if (this.id !== 'new') {
      this.fStore.gettemplate(query);
  ...
  update(data: ITemplate): void {
    this.fStore.updateTemplate(this.id, data);
    this.toggleEditMode(false);
  }

  deleteD(): void {
    this.fStore.deleteTemplate(this.id);
    this.toggleEditMode(false);
