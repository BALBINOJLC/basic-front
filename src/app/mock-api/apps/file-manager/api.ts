...
export class FileManagerMockApi {
  ...
  registerHandlers(): void {
    ...
    this._fuseMockApiService.onGet('api/apps/file-manager').reply(({ request }) => {
      ...
      if (currentFolder) {
        let folderId = currentFolder.folderId;

        while (folderId) {
          const foundFolder = pathItems.find((item) => item.id === folderId);

          if (foundFolder) {
            path.unshift(foundFolder);
            folderId = foundFolder.folderId; // Actualiza el folderId para la próxima iteración
          } else {
            // Salir del bucle si no se encuentra el folderId en pathItems
            break;
          }
        }
      }
