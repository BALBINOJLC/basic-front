...
export class ScrumboardMockApi {
  ...
  registerHandlers(): void {
    ...
    this._fuseMockApiService.onPatch('api/apps/scrumboard/board/card').reply(({ request }) => {
      ...
      // Check if updatedCard is not null
      if (updatedCard !== null) {
        // Attach the labels of the card
        updatedCard.labels = updatedCard.labels
          .map((cardLabelId: any) => this._labels.find((label) => label.id === cardLabelId))
          .filter((label: undefined) => label !== undefined);

        // Return the updated card
        return [200, updatedCard];
      } else {
        // Handle the case where the card was not found and updatedCard is null
        // For example, you can return a 404 Not Found status
    ...
    this._fuseMockApiService.onPatch('api/apps/scrumboard/board/card/positions').reply(({ request }) => {
      ...
      this._cards.forEach((card) => {
        ...
        card.position = cards.findIndex(
          (item: { id: any; listId: any; boardId: any }) =>
